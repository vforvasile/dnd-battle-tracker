    ...
    <>
      {displayLong && (
        <div className={className}>
          <b>Max Hit Points</b> {maxHp}
        </div>
      )}
      {displayLong && displayTempHp && (
        <div className={className}>
          <b>Temporary Hit Points</b> {tempHp}
        </div>
      )}
      {armorClass && (
        <div style={{ marginRight: 15 }} className={classes}>
          {short && <b>{`AC ${armorClass} `} </b>}
          {!short && (
            <>
              <b>{acLabel}</b> {armorClass}{" "}
            </>
          )}
        </div>
      )}
