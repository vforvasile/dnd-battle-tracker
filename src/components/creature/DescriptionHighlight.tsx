...
import { DamageTypesObject, DamageType } from "../../util/characterSheet";
...
function DescriptionHighlight({ text }: Props) {
  ...
  return (
    <>
      {splitText.map((item, index) => {
        ...
        const filterDamageTypes = foundDamages
          .map((currentDamage) => currentDamage.replace(" damage", ""))
          .filter(
            (thisDamage) => thisDamage.toLowerCase() in DamageTypesObject,
          );
        ...
        const newSentence = sentence
          .map((word) => {
            if (finalDamageTypesArr.includes(word)) {
              const newWord = word as DamageType;
              return `${word} ${DamageTypesObject[newWord]}`;
            }
            return word;
          })
          .join(" ");
        ...
        return (
          <Highlighter
            ...
            activeClassName={`${defaultClassName} ${
              isAttackType ? "attack-highlight" : "damage-highlight"
            }`}
