...
export default function SpellStat({ description, spellData }: Props) {
  if (!spellData) {
    return (
      <div className="property-block">
        ...
        {description && (
          <p>
            <DescriptionHighlight text={description} />{" "}
          </p>
        )}
      </div>
    );
  ...
  return (
    <div>
      <div className="property-block">
        ...
        {spellHeader && (
          <p>
            <DescriptionHighlight text={spellHeader} />
          </p>
        )}
      </div>
      <br />
      <p id="spell-level-text">
        {spellData.spells.map((spellThread) => {
          const levelName =
            spellThread.level === "0"
              ? "Cantrips"
              : ` ${spellThread.level}${getSpellLevelPrefix(
                  spellThread.level,
                )} level`;
          const suffix =
            spellThread.level === "0"
              ? "(at will):"
              : `(${spellThread.slots.length} slots):`;
          return (
            <span key={`Level-${spellThread.level}`}>
              <span>
                <b>{levelName}</b> {suffix}{" "}
              </span>
              {spellThread.knownSpells && (
                <SpellList knownSpells={spellThread.knownSpells} />
              )}
              <br />
            </span>
          );
